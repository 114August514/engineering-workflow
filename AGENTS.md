...
反复会有人问：为什么不把「前端书写」「后端书写」「仓库初始化」「前后端对接」
拆成独立 skill？测过了，答案是**不拆**：

- 19 份 references 之间有 48 条交叉引用，按话题切一刀会有 **18 条（38%）跨界**
- 被引最多的三份恰恰是**横切**的：`proportion.md`（12 次）、`audit.md`（9 次）、
  `reuse.md`（6 次）—— 它们对前端后端初始化一视同仁
- 拆了只有两条路：断掉那 18 条引用，或把这三份复制进每个 skill。
  后者违反本仓库自己的规则（无机器检查的重复一律禁止）
- `contract.md` 的全部价值就是**它是前后端的接缝**，拆开等于把缝变成跨 skill 问题
- 省 token 的理由不成立：SKILL.md 只有 ~180 行，references 本来就按需读

**真正该解决的是路由**，不是文件组织：SKILL.md 顶部有「快速路径」，
窄请求（写个组件/改个接口/修个 bug）直接跳技术专题，不走阶段机。

如果将来发现触发确实不准，可选方案是**薄壳卫星 skill**——
描述窄、正文两行、只负责转发到本 skill 的某份 reference。
但那要等到有真实的误触发证据再做，别提前建。
